...
& {
    ...
    $system_identifiers | Add-Member `
        ...
        -Name Runtime `
        -Value (& {
            # Don't parse RuntimeInformation.RuntimeIdentifier on Windows, because it doesn't parse Win11 correctly
            If( -not [System.Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([System.Runtime.InteropServices.OSPlatform]::Windows) ){
                Try {
                    [System.Runtime.InteropServices.RuntimeInformation]::RuntimeIdentifier.ToString()
                } Catch {}
            }
        })


    If( -not $system_identifiers.Runtime ) {
        ...
        $rids | ForEach-Object {
            ...
            $system_identifiers.GraphRuntimes( $_, $graphs[$_][0], $graphs[$_] )
        }
